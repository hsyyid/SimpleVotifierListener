...
public class PaginatedList
{
	...
	public Text getPage(int pageNumber)
	{
		...
		for (Text item : items)
		{
			...
			{
				list.append(Text.of(this.lineNumberColor, (itemIndex < 10) ? "0" + itemIndex + this.lineNumberType : Integer.toString(itemIndex) + this.lineNumberType));
			}
	...
	private Text getLink(String preview, int page)
	{
		...
		return Text.builder(preview).onClick(TextActions.runCommand(this.command + " " + page)).onHover(TextActions.showText(Text.of(TextColors.WHITE, "Go to page ", TextColors.GOLD, page))).color(this.clickableLinkColor).build();
